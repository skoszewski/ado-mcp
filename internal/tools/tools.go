...
func Register(server *mcp.Server, client *ado.Client, maxLogLines int, optimizations Optimizations) []string {
	...
	return []string{
		...
		addTool(h, server, "ado_list_run_changes", listRunChangesDescription, nil, h.listRunChanges),
		addTool(h, server, "ado_get_diff", getDiffDescription,
			map[string]any{"base_ref_type": "commit", "target_ref_type": "commit"}, h.getDiff),
		addTool(h, server, "ado_get_pipeline_yaml", getPipelineYAMLDescription,
			map[string]any{"start_line": 1, "line_count": defaultLineCount}, h.getPipelineYAML),
		addTool(h, server, "ado_list_run_artifacts", listRunArtifactsDescription, nil, h.listRunArtifacts),
		addTool(h, server, "ado_list_service_connections", listServiceConnectionsDescription, nil, h.listServiceConnections),
		addTool(h, server, "ado_list_service_connection_history", listServiceConnectionHistoryDescription,
			map[string]any{"top": defaultRunCount}, h.listServiceConnectionHistory),
		addTool(h, server, "ado_list_variable_groups", listVariableGroupsDescription, nil, h.listVariableGroups),
		addTool(h, server, "ado_list_environments", listEnvironmentsDescription, nil, h.listEnvironments),
		addTool(h, server, "ado_list_environment_deployments", listEnvironmentDeploymentsDescription,
			map[string]any{"top": defaultRunCount}, h.listEnvironmentDeployments),
		addTool(h, server, "ado_find_pull_requests", findPullRequestsDescription,
			map[string]any{"top": defaultRunCount}, h.findPullRequests),
		addTool(h, server, "ado_list_pull_request_threads", listPullRequestThreadsDescription, nil, h.listPullRequestThreads),
		addTool(h, server, "ado_list_policy_evaluations", listPolicyEvaluationsDescription, nil, h.listPolicyEvaluations),
		addTool(h, server, "ado_list_agent_pools", listAgentPoolsDescription, nil, h.listAgentPools),
		addTool(h, server, "ado_list_agents", listAgentsDescription, nil, h.listAgents),
		addTool(h, server, "ado_list_refs", listRefsDescription, nil, h.listRefs),
		addTool(h, server, "ado_search_code", searchCodeDescription, map[string]any{"top": defaultRunCount}, h.searchCode),
	}
}
